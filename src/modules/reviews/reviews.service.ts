...
export class ReviewsService {
  ...
  /**
   * Get all reviews for seller's products only (SELLER ISOLATION)
   */
  async findAllForSeller(
    ...
    try {
      ...
      // TENANT ISOLATION: Filter reviews by seller's products only
      ...
      this.logger.log(
        `Seller ${sellerContext.sellerId} retrieved ${total} reviews for their products`,
      );
      ...
      this.logger.error(
        `Error fetching seller reviews: ${error.message}`,
        error.stack,
      );
