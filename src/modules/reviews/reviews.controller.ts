...
export class ReviewsController {
  ...
  // Seller endpoint - Get reviews for seller's products only
  @Get('seller/all')
  @UseGuards(AuthGuard('jwt'), RolesGuard)
  @Roles(Role.SELLER, Role.ADMIN)
  @ApiBearerAuth()
  @ApiOperation({
    summary: '[SELLER] Get reviews for own products',
    description:
      'Seller endpoint to retrieve reviews only for their own products',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiQuery({ type: ReviewFilterDto })
  async getAllReviewsSeller(
    @Query() filterDto: ReviewFilterDto,
    @GetSellerContext() sellerContext: SellerContext,
  ): Promise<PaginatedReviewResponseDto> {
    return this.reviewsService.findAllForSeller(filterDto, sellerContext);
  }
