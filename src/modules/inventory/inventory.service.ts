...
export class InventoryService {
  ...
  async findAll(sellerContext?: SellerContext): Promise<InventoryResponseDto[]> {
    try {
      // Build where clause for seller filtering
  ...
  async getLowStockItems(sellerContext?: SellerContext): Promise<InventoryResponseDto[]> {
    try {
      // Build where clause for seller filtering
      const where: Prisma.InventoryWhereInput = {};
      ...
      // Get all inventories with seller filter applied
      const inventories = await this.prismaService.inventory.findMany({
        where,
        include: {
          product: {
