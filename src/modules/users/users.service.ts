...
export class UsersService {
  ...
  async updateUserStatus(
    ...
    try {
      ...
      // Note: We don't have current admin ID here, so this check would need to be in controller
      
      this.logger.log(`Updating user status: ${user.email} → ${status}`);
      ...
      this.logger.error(
        `Error updating user status for ID ${id}: ${error.message}`,
        error.stack,
      );
  ...
  async updateUserRole(
    ...
    try {
      ...
      // Prevent changing role if user already has that role
      if (user.role === role) {
        throw new BadRequestException(`User already has role: ${role}`);
      }

      this.logger.log(`Updating user role: ${user.email} (${user.role} → ${role})`);
      ...
      this.logger.error(
        `Error updating user role for ID ${id}: ${error.message}`,
        error.stack,
      );
  ...
  async deleteUser(id: string): Promise<void> {
    try {
      ...
      if (user.role === Role.ADMIN) {
        throw new BadRequestException('Cannot delete admin users for security reasons');
      }

      this.logger.warn(`Deleting user: ${user.email} (${user.role})`);
