...
/** Product shape used for coupon eligibility (price, category) */
interface CartProductForCoupon {
  id: string;
  price: number | string;
  ...
  async applyCoupon(
    ...
    if (coupon.type === 'PERCENTAGE') {
      ...
      if (
        ...
        // Create a map for easy lookup (typed for eligibility checks)
        const productMap = new Map<string, CartProductForCoupon>(
          (products as CartProductForCoupon[]).map((p) => [p.id, p]),
        );
