...
export class ProductsController {
  ...
  @ApiOperation({ summary: 'Create a new product (Admin and Seller)' })
  ...
  async create(
    ...
    // Determine sellerId and storeId based on role
    let sellerId: string;
    let storeId: string;

    if (sellerContext.isAdmin) {
      // Admin must specify both sellerId and storeId
      if (!createProductDto.sellerId) {
        throw new BadRequestException(
          'Admin must provide sellerId when creating products',
        );
      }
      sellerId = createProductDto.sellerId;
      storeId = createProductDto.storeId; // Already validated as required in DTO
      
      // Verify the store exists and belongs to the specified seller
      const store = await this.productsService['prismaService'].store.findUnique({
        where: { id: storeId },
      ...
      if (store.sellerId !== sellerId) {
        throw new BadRequestException(
          `Store ${storeId} does not belong to seller ${sellerId}`,
        );
      }
    } else if (sellerContext.isSeller) {
      // Seller: auto-assign sellerId from their context
      if (!sellerContext.sellerId) {
        throw new ForbiddenException('Seller ID not found in context');
      }
      
      sellerId = sellerContext.sellerId;
      storeId = createProductDto.storeId; // Seller provides storeId
      
      // Verify the seller owns the store they're trying to add products to
      const store = await this.productsService['prismaService'].store.findUnique({
        where: { id: storeId },
        select: { sellerId: true },
      });
    ...
    return this.productsService.create(createProductDto, sellerId, storeId);
  }
  ...
  @ApiOperation({ summary: 'Update a product (Admin and Seller)' })
  @ApiParam({
    name: 'id',
    required: true,
  ...
  @Roles(Role.ADMIN, Role.SELLER)
  @ResourceType('product')
  @ApiOperation({ summary: 'Delete a product (Admin and Seller)' })
  @ApiParam({
    name: 'id',
    required: true,
