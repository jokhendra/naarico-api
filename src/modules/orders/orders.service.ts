...
export class OrdersService {
  ...
  async findAll(
    ...
    try {
      ...
      // TENANT ISOLATION: Filter by store/seller if not admin
    ...
    try {
      ...
      const result = await this.prismaService.$transaction(
        async (prisma) => {
          ...
          // Validate that all products belong to the same store (for multi-tenancy)
          const storeIds = Array.from(
            new Set(itemsWithPrices.map((item) => item.storeId)),
          );

          if (storeIds.length > 1) {
            throw new BadRequestException(
              'Order cannot contain products from multiple stores. Please create separate orders for each store.',
            );
          }

          const orderStoreId = storeIds[0];
          ...
          const order = await prisma.order.create({
            data: {
              ...
              storeId: orderStoreId, // Auto-assign storeId from products
