    ...
    try {
      ...
      if (!placeholderProduct) {
        try {
          ...
          // Get or create system admin and store for placeholder products
          ...
          if (!systemAdmin) {
            throw new BadRequestException(
              'Cannot create deal: No admin user found in system. Please run seed data first.',
            );
          }

          // Get or create system store
          let systemStore = await this.prisma.store.findFirst({
            where: { sellerId: systemAdmin.id },
          });
          ...
          if (!systemStore) {
            systemStore = await this.prisma.store.create({
              data: {
                name: 'System Store',
                slug: 'system-store-' + Date.now(),
                description: 'System store for internal operations',
                sellerId: systemAdmin.id,
                status: 'APPROVED',
              },
            });
          }
