import { PrismaClient, Prisma, ProductVisibility, DealType, InventoryChangeType, Role, UserStatus } from '@prisma/client';
...
async function ensureDefaultSellerAndStore() {
  ...
  return { seller: admin, store };
}

async function upsertProducts(products: GeneratedProduct[], sellerId: string, storeId: string) {
  // Ensure tags exist
...
export async function seedProducts(prismaClient: PrismaClient = prisma): Promise<void> {
  ...
  // Ensure default seller and store exist
  const { seller, store } = await ensureDefaultSellerAndStore();
  console.log(`Using seller: ${seller.email}, store: ${store.name}`);

  const products = generateProducts(brands, categories, 3); // adjust density here
  await upsertProducts(products, seller.id, store.id);
